...
def get_timetables(html) -> Dict:
    ...
    for day in day_divs:
        ...
        for show in day.xpath("//div[contains(@class, 'timetable-column-show ')]"):
            ...
            day_result.append(show_result.copy())
            show_result.clear()

        for check_day in day_of_week:
            if check_day in current_class:
                result[check_day] = day_result.copy()
...
def save_result(result: Dict, filename: str = "anime_weekly_timetable.json", pretty_print: bool = True) -> None:
...
if __name__ == '__main__':
    ...
    json_dict = get_timetables(tree)

    num_anime = 0

    for day in json_dict:
        for i, show in enumerate(json_dict[day]):
            num_anime += 1
            if (mal_id := get_mal_id(show['raw_name'])) is not None:
                json_dict[day][i]["mal_id"] = mal_id

    logger.info(f"Processed {num_anime} shows")

    save_result(json_dict)
