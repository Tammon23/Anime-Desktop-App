...
public sealed class VlcSharpWriteableBitmap : IBitmap, IAffectsRender
{
    ...
    private readonly ISubject<Unit>? _rendered = new Subject<Unit>();
    private readonly ISubject<Unit>? _updated = new Subject<Unit>();
    ...
    {
        Read(_ => context.DrawBitmap(_read?.PlatformImpl, opacity, sourceRect, destRect, bitmapInterpolationMode));
        NotifyRendered();
    }
    ...
    public void Write(PixelSize size, Vector dpi, PixelFormat format, Action<ILockedFramebuffer> action)
    {
        using (LockWrite())
        {
            ...
            {
                _write?.Dispose();
                _write = new WriteableBitmap(size, dpi, format, AlphaFormat.Opaque);
                _pixelFormat = format;
            }
