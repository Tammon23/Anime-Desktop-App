...
public class Manga
{
    ...
    public static async Task<MangaList?> GetMangaList(string searchString, int offset = 0, int limit = 100,
        string fields = "")
    {
    ...
    public static async Task<MangaRanking?> GetAnimeRanking(MangaRankingTypeEnum rankingType, int offset = 0,
        int limit = 100,
        string fields = "")
    {
        if (limit is > 500 or < 0) throw new LimitOutOfRangeException("Limit must be between 0 and 500");

        var ranking = Util.MangaRankingTypeToString(rankingType);
